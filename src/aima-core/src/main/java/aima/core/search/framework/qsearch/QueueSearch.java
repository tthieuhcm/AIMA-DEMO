...
public abstract class QueueSearch<S, A> {
	...
	public void PrintTree(Node<S, A> node, int level){
		...
		if (node.getAction() != null) System.out.println(print + node.getAction().toString());
		else System.out.println(print + "Initial State");
		if (node.successors != null) for (Node<S, A> successor : node.successors) {
